...
# ---------------------------------------------------------------------------
# Simple in‑memory rate limiter (per API key, 5 requests / minute)
# ---------------------------------------------------------------------------
rate_limit_store: dict[str, list[float]] = {}

def _check_rate_limit(api_key: str) -> None:
    """Raise HTTPException 429 if the key has made > 5 requests in the last 60s."""
    now = datetime.utcnow().timestamp()
    if api_key not in rate_limit_store:
        rate_limit_store[api_key] = []
    # Remove timestamps outside the current window
    rate_limit_store[api_key] = [
        t for t in rate_limit_store[api_key] if now - t < 60
    ]
    if len(rate_limit_store[api_key]) >= 5:
        raise HTTPException(429, "Rate limit exceeded. Max 5 requests per minute.")
    rate_limit_store[api_key].append(now)
...
# --- Video Generation (calls your existing pipeline) ---
async def run_generation(job_id: str, request: GenerateRequest) -> None:
    job_file = JOBS_DIR / f"{job_id}.json"
...
async def generate_video(
    ...
    _check_rate_limit(user_id)  # rate limit per API key
    if not check_credits(user_id, amount=1):
        raise HTTPException(402, "Insufficient credits")
